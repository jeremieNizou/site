...
myApp.controller("todolistPresCtrl", function($scope, serviceAjax, $routeParams) {
	...
	var afficheItem = function() {
		serviceAjax.afficheItem($routeParams.id).then(function onSuccess(response) {
			if( response === "err" ) {
...
// Fonction modifier presentation
	...
	$scope.confirmerPres = function(item) {
		serviceAjax.modifiePres(item._id, $scope.changeTodo).then(function onSuccess(response) {
			if( response === "err" ) {
				alert("Probleme ajout");
			}
			else {
